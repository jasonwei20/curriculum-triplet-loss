...
def load_ap_data(cfg, alpha):

    if cfg.aug_type in {'sr', 'eda', 'rd', 'bt', 'nm', 'ms', 'so'}:
        return load_ap_data_aug(cfg, alpha)
    else:
        return load_ap_data_no_aug(cfg)

def load_ap_data_no_aug(cfg, seed_num):
    ...
    test_sentence_to_label = get_test_subset(test_sentence_to_label, cfg.val_subset, seed_num)

    if cfg.train_nc:
        train_label_to_sentences = get_train_subset(train_label_to_sentences, cfg.train_nc, seed_num)
